...
management login to `NETWORKGAMES-SETUP.txt` on the boot partition. These
...
with the management password from `NETWORKGAMES-SETUP.txt`, then open
...
To force recovery mode, create an empty `networkgames-recovery` file on the
