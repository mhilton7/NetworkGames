...
image=networkgames-host:${version}
container=networkgames-host-test
mkdir -p reports
mkdir -p "$root"/{library,config,data,certs,logs,backups}
GOCACHE="${GOCACHE:-/tmp/networkgames-go-cache}" \
GOPATH="${GOPATH:-/tmp/networkgames-gopath}" make server
chmod u+w build/container-root/networkgames-host
cp build/bin/networkgames-host build/container-root/networkgames-host
...
openssl req -x509 -newkey rsa:2048 -nodes -days 2 -subj /CN=networkgames-test-ca \
...
sudo docker run -d --name "$container" \
  ...
  -e NETWORKGAMES_ADMIN_TOKEN=0123456789abcdef0123456789abcdef \
...
sudo docker exec "$container" /networkgames-host healthcheck
if sudo docker exec "$container" /networkgames-host scan --library /library >/dev/null; then :; else exit 1; fi
if sudo docker exec "$container" /networkgames-host version >/dev/null; then :; fi
