...
WIIBRIDGE_LIBRARY="$root/library" WIIBRIDGE_DATA="$root/data" \
WIIBRIDGE_ADMIN_TOKEN=0123456789abcdef0123456789abcdef \
WIIBRIDGE_TLS_CERT="$root/certs/server.crt" \
WIIBRIDGE_TLS_KEY="$root/certs/server.key" \
WIIBRIDGE_TLS_CLIENT_CA="$root/certs/clients-ca.crt" \
WIIBRIDGE_NBD_LISTEN="127.0.0.1:$port" \
WIIBRIDGE_HTTPS_LISTEN=127.0.0.1:21443 \
  build/bin/wiibridge-host > "$root/host.log" 2>&1 &
host_pid=$!
