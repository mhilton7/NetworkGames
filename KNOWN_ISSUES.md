...
- The TrueNAS target at `192.0.2.10` is network-reachable: TCP 8445 and
  ...
  certificate for `192.0.2.10`, and the live NBD export accepts the
...
- The original generated server certificate covered only the Pi-facing
  `192.0.2.10` address while the Compose health check uses
...
- The following physical retest still produced the link-local iPhone address
  `192.0.2.10`. Persistent logs proved that dnsmasq could not traverse the
