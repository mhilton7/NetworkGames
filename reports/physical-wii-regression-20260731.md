...
Status: correction validated locally; clean publication, deployment, and
...
Publication and deployment are pending. Before replacement, retain the current
digest shown above and export the active TrueNAS YAML/configuration. After a
clean candidate is published, pin the immutable digest, recreate the app, run
`deploy/truenas/verify-runtime-identity.sh`, re-audit the regenerated live FAT
and split sizes, then detach/reconnect NBD and USB in order.
