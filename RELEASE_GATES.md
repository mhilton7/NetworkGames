...
| GHCR WBFS attribute replacement image | PASS | Commit `74dfb5b`; `ghcr.io/mhilton7/wiibridge-host:0.1.0-rc.1-wbfsattrfix.1@sha256:37d94d0c3f11ae8c33f96490fe9c0902b6b417907afd56a14d8dd370f4b2fe80` published and anonymously resolved |
...
| Large Wii 32 KiB replacement OCI | PASS | Commit `2a7383a`; `ghcr.io/mhilton7/wiibridge-host@sha256:5538addc6e525d0ffac405192c97a3ef955eef4d9974e327e22e3e070f487b3f` published and anonymously resolved |
...
| Exact FSInfo TrueNAS deployment | PASS | Commit `8e8ec40`; `ghcr.io/mhilton7/wiibridge-host@sha256:aa1a5a6db11320c504daed2c8b198b1fb8bc6836c3a5ef90fb5b4c4adc44707f` is live and Ready. The 1,813,217,599,488-byte export reports one free cluster, points to unallocated cluster 55,321,472, and retains 32 KiB FAT32 geometry |
...
| Physical USB Loader GX clean-SD-cache isolation retest | PASS | With repaired SD metadata and isolated caches, r1283 reached the complete catalog on the corrected live export; exact six-run timing remains a separate open gate |
...
| Split-boundary clean OCI | PASS | PR build at focused commit `f9fc32c` passed; main merge `2851a568` published `ghcr.io/mhilton7/wiibridge-host@sha256:dc9a1b7b9223efca9dd174e3c8129e28330d7ded20a95d7ea2039db2b1e8bab5`; an independent pull confirms matching binary/OCI revision and `dirty false` |
| Split-boundary TrueNAS deployment | BLOCKED_EXTERNAL | Operator deployed the pinned candidate; live health reports clean revision `2851a568`, readiness passes, corrected disk geometry/fsck/segment sizes pass, and Pi reconnect passes. TrueNAS configured/running digest equality still requires management-plane access |
| Physical catalog after split correction | PASS | The complete catalog became visible and allowed game selection; this removes the prior post-enumeration `Loading resources` blocker. Exact three-cold/three-warm timings remain pending |
| Physical banner controls after split correction | FAIL | `SM2E52` displayed its animated banner, but `SLREWR` and a repeat `R22E01` selection froze before new NBD reads; 3/3 acceptance is not met |
| Physical gameplay after split correction | FAIL | Verified low-LBA `R22E01` loaded once after a delay, but `SM2E52` failed launch and `R22E01` froze on selection after Return To USB Loader; sustained three-game acceptance is not met |
| Return-To-Loader cold-relaunch isolation | PENDING | Next exact experiment: physical Wii Reset, cold r1283 relaunch, and repeat `R22E01` selection without changing Pi/TrueNAS state |
