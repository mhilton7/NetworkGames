...
| Adaptive large Wii FAT32 geometry | PASS | The builder retains 4 KiB clusters where valid and selects the first larger valid power-of-two cluster size subject to FAT32 cluster-number and MBR/LBA limits; the 1.65 TiB-scale regression selects 8 KiB and approximately 221 million data clusters |
| Large Wii FAT32 replacement OCI | PASS | Full tests, targeted race tests, static checks, Compose validation, hardened container lifecycle, and OCI inspection pass locally; dirty-worktree candidate digest `sha256:7150b81412b643c99cf996247371d4098529a4355dd9a688b743439c5e5d31fe` is not a published release |
| Corrected large Wii TrueNAS deployment | PENDING | The workspace has no authenticated TrueNAS management shell/API; the running export remains on the invalid fixed-4-KiB builder until an operator deploys a clean published replacement |
| Physical USB Loader GX large-library retest | PENDING | Requires the corrected TrueNAS Host export followed by Pi reconnect and physical Wii enumeration |
